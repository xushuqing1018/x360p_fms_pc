...
const hasPer = (nav,pers) => {
	if(!pers) return false
	return pers.indexOf(nav.uri) !== -1
}
...
/**
 * @param {Object} navs 菜单定义
 * @param {Object} paths 路由定义
 * @returns {Array}
 */
export const getMenuByUserNav = (navs, paths, pers) => {
		let res = []
		forEach(navs, nav =>{
			let item = paths[nav.uri]
			if(!item){
				return
			}
			let obj = {
			...
			//}
			if(!obj.meta.title){
				obj.meta.title = nav.text
			}
			if(hasSub(nav)){
				obj.children = getMenuByUserNav(nav.sub,paths)
			}
			if (item.meta && item.meta.href) obj.href = item.meta.href
			res.push(obj)
			//if (hasPer(nav, pers)) res.push(obj)
		})
		console.log('getMenuByUserNav====')
		console.log(res)
		return res
}
...
function week_range_time(){
    ...
    start = new Date(cur_time - (cur_week_day * 86400000))
    end   = new Date(cur_time + (6 - cur_week_day) * 86400000)
    return [start,end]
}
...
function random(length) {
    var dics = '0123456789abcdefghijklmnopqrstuvwxyz$#@~ABCDEFGHIJKLMNOPQRSTUVWXYZ',
        str = '',
...
　 let firstDay = first_week_begin_day(dateobj.getFullYear());
　 if (dateobj < firstDay) {
　　 firstDay = first_week_begin_day(dateobj.getFullYear() - 1);
　 }
