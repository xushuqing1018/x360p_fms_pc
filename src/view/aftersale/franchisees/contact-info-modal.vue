<template>
	<Modal draggable v-model="modal$.show" :title="modal$.title" :mask-closable="false" width="500">
		<Form ref="form" :model="info" :label-width="80">
			...
			<Row>
				...
				<Col span="12">
					<Form-item label="股权比例" prop="partner_percent" v-if="info.is_partner">
						<InputNumber v-model="info.partner_percent" :min="0" :max="100"></InputNumber>
						<span>(单位，%)</span>
					</Form-item>
				</Col>
			</Row>
			...
			<Form-item label="主联系人" prop="is_main">
				<i-switch v-model="info.is_main" :true-value="1" :false-value="0">
					<span slot="open">是</span>
					<span slot="close">否</span>
				</i-switch>
			</Form-item>
		...
		methods: {
			...
			ok() {
				...
				if(this.info.mobile==='') {
					...
					if(!(/^1\d{10}$/).test(this.info.mobile)) {
