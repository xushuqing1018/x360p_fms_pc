<template>
	<div class="ticket-chat">
		<div class="header">
			<a @click="geTicketList"><Icon type="ios-arrow-back" size="16" />工单列表</a>
		...
		<div class="content">
			...
			<div class="submit">
				<Button type="primary" @click="receive" :loading="saving" v-if="data.status==0">我要受理</Button>
				<template v-else-if="data.status!==3">
					<Button type="primary" @click="reply" :loading="saving">我要回复</Button>
					<Button type="text" @click="close" :loading="saving">确认结单</Button>
				</template>
			</div>
		</div>
	...
	export default {
		...
		methods: {
			receive() {
				...
				.add_url_param(this.data.fsa_id,'doreceive')
				.post()
				.success(res => {
					this.get_apply_info()
				})
			...
			reply() {
				...
				.then(modal => {
					modal
					.show(util.sprintf('To：%s',this.data.fc_name),'add')
				})
			},
			close() {
			...
			geTicketList() {
				...
				this.$router.push('/aftersale/tickets')
			},
			get_apply_info() {
				this.loading = true
