<template>
	<Modal draggable v-model="modal$.show" :title="modal$.title" :width="800" :mask-closable="false">
		<div class="select-list" v-if="modal$.show">
			<div class="list-condition">
				<Form class="row mb-2" ref="form" :model="search_field" :label-width="60">
				...
				<Button type="primary" icon="md-refresh" size="small" class="ml-2" @click="reset">重置</Button>
			</div>
			<div class="list-body">
...
<script>
	...
	export default{
		...
		methods: {
			domktoken (item) {
				this.$Modal.open('aftersale/x360s/user-token-modal.vue')
				.then(modal=>{					
					modal
					.set('uid',item.uid)
