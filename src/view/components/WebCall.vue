	...
	<div ref="dragEl" class="x-call-container" :class="callClass" v-if="enableWebCall" v-drag>
		<div class="normal">
			<div class="x-call-left call-member">
				...
				<div class="avatar">
					<avatar size="small" icon="ios-person" :src="caller.info.avatar"></avatar>
					<Icon title="隐藏并清除拨号信息" @click.native="clearAndClose" class="avatar-icon" type="minus-circled"></Icon>
				</div>
				<span class="tel">{{caller.info.mobile}}</span>
			</div>
			...
			<div class="x-call-right call-member">
				...
				<div class="avatar">
					<avatar size="small" icon="ios-person" :src="receiver.photo_url"></avatar><br>
...
<script>
	...
	export default{
		...
		computed: {
			...
			operateIcon () {
				return `http://sp1.xiao360.com/static/img/call-customer.png`
			},
			callClass () {
				return {
					'min': !this.modal$.show
		...
		methods: {
			...
			makeCall (id) {
				//callee_type:1市场名单 ,2客户,3学生,4加盟商
				let params = {
			...
			setReceiver (selected) {
				...
				if(sence=='customer'){
					...
					this.receiver.name = selected.name
					this.receiver.photo_url = ''
					this.receiver.tel = selected.mobile
					this.receiver.real_tel = ''
					this.receiver.callee_type = 4
					id = {'fcp_id':selected.fcp_id}
