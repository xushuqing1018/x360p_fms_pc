<template>
	<Button type="primary" @click="bat_add" icon="ios-cloud-upload"><slot>批量导入</slot>
		<Modal draggable :mask-closable="false" v-model="modal" :title="modalTitle">
            <Steps :current="step">
            ...
            <div class="import-wrap">
                <div class="down-desc" v-if="step == 0">
                    ...
                    <a :href="tpl" @click="import_next" download class="ivu-btn ivu-btn-info"><Icon type="ios-cloud-download-outline"></Icon>下载</a>
                </div>
                <div v-if="step == 1">
                    <Upload
                        type="drag"
                    ...
                    <div v-if="imported">
                        ...
                        <div class="well" v-if="records.failure > 0 || records.repeat > 0">
                            ...
                            <div class="logs-container">
                                <p v-for="log in records.error_logs">{{log}}</p>
                            </div>
                        </div>
                        <p>
                            <button class="ivu-btn ivu-btn-info" @click="go_step(0)">返回再次导入</button>
	...
	export default {
        ...
        methods:{
            ...
            loopDealData(){
                ...
                this.$http.get('fapi/import?func='+this.res+params+'&fk='+this.records.fk+'&page='+this.records.page+'&pagesize=20')
                .then(response=>{
            ...
            endImport(result){
                ...
                this.$http.delete('fapi/import')
            }
        },
        computed:{
        	tpl(){
        		let params = this.params?'&'+this.params:''
        		return '/fapi/import?tpl='+this.res+'&token='+this.$store.state.user.token+params
        	},
        	modalTitle(){
