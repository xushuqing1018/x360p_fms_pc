<template>
	<Modal draggable v-model="modal$.show" :title="modal$.title" width="500" :mask-closable="false">		
		<div style="overflow: auto">
			...
			<div class="text-center" v-if="file_list('image').length>0">
				<div class="review-upload-list mb-2" v-for="(item,index) in file_list('image')">
					<a :href="item.file_url" target="_blank"><img :src="item.file_url"/></a>
				</div>
			</div>
			<div class="text-center" v-if="file_list('video').length>0">			
			    <div 
			    	class="review-upload-list mb-2" 
			    	:class="index>0?'ml-3':''" 
			    	v-for="(item,index) in file_list('video')" 
			    	:key="index"
			    	>
			...
			<div class="pd-10 text-center" v-if="file_list('file').length>0">
				<div 
					class="review-upload-list mb-2" 
					:class="index>0?'ml-3':''" 
					v-for="(item,index) in file_list('file')" 
					:key="index"
					>
