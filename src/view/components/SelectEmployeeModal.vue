...
<template>
    <Modal draggable v-model="modal$.show" :mask-closable="true" :width="width" :title="title">
    	<div :class="classes">
    		<div class="list-main">
                <div class="row">
                    <div class="col-sm-4">
                        <div class="tree-wrap">
                            ...
                            <x-tree
                                v-show="authRoles.length>0&&rid==0"
                                :data="authRoles"
            ...
            <div class="multi-selected" v-if="this.multiple">
                <Card>
                    <div slot="title">
                        ...
                        <Button @click="clearSelectedId" class="pull-right" shape="circle" size="small" icon="ios-trash" title="清空"></Button>
                    </div>
                    <ul class="multi-selected-list">
                        <li v-for="item in selectedList">
                            <span>{{item.ename}}</span>
                            <Button @click="cancelSelectedId(item)" class="pull-right" shape="circle" size="small" icon="ios-trash" title="移除"></Button>
                        </li>
                    </ul>
                </Card>
            </div>
...
export default {
	name:'EmployeeList',
    mixins: [common,modal,globals,globalselect],
    components:{
        XTree,
        AvatarAndName
    },
    ...
    methods: {  
        ...
        getAuthRolesTree() {
            ...
            roles.forEach(item=>{
                item.id = item.rid
                item.pid = 0
                item.icon = 'ivu-icon ivu-icon-md-man'
            })
            return roles
        },
        getSubjectsTree() {
