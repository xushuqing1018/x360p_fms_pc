...
<template>
	<Modal draggable v-model="modal$.show" :title="modal$.title" :width="800" :mask-closable="false" @on-cancel="cancel">
		<template v-if="modal$.show">
			...
			<review-upload class="mt-3" ref="upload" v-model="post.review_file" :mode="modal$.action"></review-upload>
					...
					<div class="row b-b-1" style="padding: 15px 0">
						...
						<template v-else>
							<div class="review-item col-md-12">
								...
								<div class="score-box">
									<div v-for="s in 10" v-if="post.review_style == 0&&item['score'+((s-1)>0?(s-1):'')]>0">
										{{scoreLabel(0,s-1)}}: <Rate v-model="item['score'+((s-1)>0?(s-1):'')]" disabled></Rate>
									</div>
									<div v-for="s in 5" v-if="post.review_style == 1&&item.detail['score'+(s-1)]>0">									
										{{scoreLabel(1,s-1)}}: <Rate v-model="item['score'+(s-1)]" disabled></Rate>
									</div>
								</div>
								<dir class="week-box" v-if="post.review_style == 1">
									<div v-for="s in 5" v-if="item.detail['weak'+(s-1)].value>0" class="week-item">								
										<Tag type="border" color="red">{{weakLabel(s-1)}}</Tag>
										{{item.detail['weak'+(s-1)].desc}}
									</div>
								</dir>
								<div class="review-item-content" style="margin-top: 5px" v-if="post.review_style == 0">{{item.detail}}</div>
								<div class="review-item-content" style="margin-top: 5px" v-if="post.review_style == 1">{{item.detail.detail}}</div>
							</div>
						</template>
					</div>
				</div>
		...
		<div slot="footer">
			<Button type="text" @click="cancel" v-if="scene=='view'">关闭</Button>
			<div v-else>
		...
		<Modal v-drag-modal v-model="modal_qr.show" width="360" :title="modal_qr.title">
			...
			<div slot="footer">
				<Button type="text" @click="modal_qr.show=false">关闭</Button>
			</div>
		</Modal>
	</Modal>
</template>
