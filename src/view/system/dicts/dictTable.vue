	...
	<Card :bordered="false">
		...
		<div class="c-grid">
			<div class="box box-result">
	            ...
	            <div class="toolbar">
	                <Button :disabled="is_add_disabled" type="primary" size="small" @click="add" icon="md-add">新增</Button>
	                <Poptip
				        confirm
				        title="您确认要进行删除操作吗？"
				        @on-ok="del"
				        @on-cancel="cancel">
				        <Button type="error" size="small" icon="md-close" :disabled="selected.none">删除</Button>
				    </Poptip>
				    <span>
					    <Alert class="m-t mt-3" type="info" show-icon v-if="is_sort_enable">排序值数字越大越靠前<span v-if="is_value_must_number">，值必须为数字</span>
					</Alert>
					</span>
	            </div>
	            <div class="content">
	                <div class="content-body" v-loading.like="'dictionarys'" style="max-height: 500px;overflow: auto">
	                	<table class="table">
	                		<thead>
	                			<tr>
	                		...
	                		<tbody>
	                			...
	                			<tr v-for="item in dataList" v-if="dataList.length > 0">
	                				...
	                				<td v-else>
	                					<Button size="small" @click="edit(item)" icon="md-create"></Button>
	                				</td>
	                			</tr>
		...
		methods:{
			...
			save(item){
				...
				if(!item.target){
					...
					$rest('dictionarys').add_url_param(item.did)
					.put(item)
					.success(response=>{
						this.clear()
						this.init_data()
			...
			init_data(){
				$rest('dictionarys').get({pid:this.pid}).success(response=>{
